...
export function useTeacherReports(filters: ReportFilters = {}) {
  return useQuery<TeacherReportsResponse>({
    ...
    queryFn: async () => {
      ...
      const url = `/api/teacher-reports${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
