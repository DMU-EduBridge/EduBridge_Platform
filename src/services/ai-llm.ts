import { RetryManager } from '@/lib/ai-server/retry-manager';
import { logger } from '@/lib/monitoring';

const getBaseUrl = () => {
  if (typeof window !== 'undefined') {
    // Client-side
    return '';
  }
  // Server-side
  return process.env.NEXTAUTH_URL || 'http://localhost:3000';
};

export const aiLLMService = {
  analyzeStudentPerformance: async (userId: string) => {
    const result = await RetryManager.executeWithRetry(async () => {
      const res = await fetch(`${getBaseUrl()}/api/ai/bridge/analyze-student-performance`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: userId }),
      });

      if (!res.ok) {
        const text = await res.text().catch(() => '');
        let message = '학습 성과 분석 호출 실패';
        try {
          const json = JSON.parse(text);
          message = json?.error || json?.detail || message;
        } catch {}
        throw new Error(message);
      }

      const data = await res.json();
      logger.info('Student performance analysis completed', {
        userId,
        success: true,
      });
      return data;
    }, RetryManager.getAIServiceConfig());

    if (!result.success) {
      logger.error('Student performance analysis failed', result.error, { userId });
      throw result.error || new Error('학습 성과 분석 호출 실패');
    }

    return result.data;
  },

  chatMessage: async (params: { userId: string; userMessage: string; history?: any[] }) => {
    const result = await RetryManager.executeWithRetry(async () => {
      const invoke = () =>
        fetch(`${getBaseUrl()}/api/ai/bridge/chat/message`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            user_id: params.userId,
            user_message: params.userMessage,
            history: params.history ?? [],
          }),
        });

      const res = await invoke();

      // 브리지 라우트는 개발 모드에서 항상 200 상태 코드로 응답하므로
      // res.ok 체크를 제거하고 직접 JSON 파싱
      const data = await res.json();

      // 에러 응답인 경우에만 처리
      if (!res.ok) {
        const detail = data?.detail || data?.error || '챗봇 메시지 호출 실패';
        const needsAnalysis =
          res.status === 404 && /analysis context|generate a report/i.test(detail);

        if (needsAnalysis) {
          try {
            await aiLLMService.analyzeStudentPerformance(params.userId);
            const retryRes = await invoke();
            const retryData = await retryRes.json();
            if (retryRes.ok) return retryData;
          } catch {
            // ignore and fallthrough
          }
        }
        throw new Error(detail);
      }

      logger.info('Chat message completed', {
        userId: params.userId,
        messageLength: params.userMessage.length,
        historyLength: params.history?.length || 0,
      });

      return data;
    }, RetryManager.getChatServiceConfig());

    if (!result.success) {
      logger.error('Chat message failed', result.error, {
        userId: params.userId,
        messageLength: params.userMessage.length,
      });
      throw result.error || new Error('챗봇 메시지 호출 실패');
    }

    return result.data;
  },
};
